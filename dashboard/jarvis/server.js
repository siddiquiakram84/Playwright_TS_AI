...
// Use process.cwd() so the server resolves paths relative to the project root
// regardless of where server.js lives. Works both locally and in Docker.
const RESULTS_FILE = process.env.RESULTS_FILE  || path.resolve(process.cwd(), 'dashboard/playwright/test-results/results.json');
const ALLURE_DIR   = process.env.ALLURE_DIR    || path.resolve(process.cwd(), 'dashboard/allure/results');
const HISTORY_FILE = process.env.HISTORY_FILE  || path.resolve(__dirname, 'metrics-history.json');

// ─── Parse Playwright JSON report ─────────────────────────────────────────────
function parseResults() {
  if (!fs.existsSync(RESULTS_FILE)) {
    return { status: 'no_data', message: 'No results.json found. Run tests first.' };
  }
  ...
  const walk = (suite, projectName = 'unknown') => {
    ...
    for (const test of tests) {
      ...
      else if (status === 'failed')   { projectMap[proj].failed++; failures.push({ title, suite: suiteName, project: proj, duration: dur, status }); }
...
function appendHistory(summary) {
  ...
  if (!last || last.startTime !== summary.startTime) {
    ...
    } catch { /* read-only mount in Docker — history lives in memory only */ }
  }
}
...
// ─── Seed history on startup so the trend chart has data immediately ──────────
function seedHistoryOnStartup() {
  ...
  try {
    ...
    const flaky   = stats.flaky      ?? 0;
    const passRate = total > 0 ? Math.round((passed / total) * 100 * 10) / 10 : 0;
    appendHistory({ total, passed, failed, skipped, flaky, passRate, duration: stats.duration ?? 0, startTime: stats.startTime ?? new Date().toISOString() });
