...
export class CartPage extends BasePage {
  ...
  get url(): string { return '/view_cart'; }
  ...
  async proceedToCheckout(): Promise<void> { await this.proceedToCheckoutBtn.click(); }

  async handleAuthModal(action: 'continue' | 'checkout'): Promise<void> {
    if (action === 'continue') await this.modalContinueBtn.click();
    else                        await this.modalCheckoutBtn.click();
  }
